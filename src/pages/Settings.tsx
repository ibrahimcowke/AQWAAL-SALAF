...
import { Sun, Moon, BookOpen, Scaling, Globe, Shield, Github, Mail, Bell, ChevronLeft } from 'lucide-react';
import { useThemeStore } from '../stores/themeStore';
import { useTranslation } from 'react-i18next';
import toast from 'react-hot-toast';

export default function Settings() {
  const { mode, setMode, fontSize, setFontSize, readingMode, toggleReadingMode } = useThemeStore();
    ...
    <div className="page-container max-w-2xl">
        ...
        <div className="space-y-4">
          <div className="neu-card p-5">
            ...
            <div className="flex gap-2">
                {[
                  ...
                  { id: 'paper', label: t('theme_paper'), icon: BookOpen },
                ].map((item) => (
                  <button
                    key={item.id}
                    ...
                    className={`flex-1 flex flex-col items-center gap-2 py-4 rounded-2xl transition-all border-2 ${
                      mode === item.id
                        ...
                        : 'border-transparent bg-[var(--color-bg-alt)] text-[var(--color-text-muted)]'
                    }`}
                  >
                    <item.icon size={24} />
                    <span className="arabic-text text-xs font-bold">{item.label}</span>
                  </button>
                ))}
            </div>
          </div>
          ...
          <div className="neu-card p-5 flex items-center justify-between">
              ...
              <button 
                ...
                className={`w-12 h-6 rounded-full transition-colors relative ${readingMode ? 'bg-[var(--color-primary)]' : 'bg-gray-300'}`}
              >
      ...
      <section className="mb-12">
          <button 
            ...
            className="w-full py-4 rounded-2xl bg-red-50 text-red-600 arabic-text font-bold text-sm shadow-sm active:scale-[0.98] transition-all"
          >
              {t('clear_data')}
          </button>
